...
df[['cholesterol', 'gluc']] = df[['cholesterol', 'gluc']].map(standardize_value)

# 4
def draw_cat_plot():
    # 5
    df_cat = pd.melt(df, id_vars=['cardio'], value_vars=['cholesterol', 'gluc', 'smoke', 'overweight', 'alco', 'active'])
